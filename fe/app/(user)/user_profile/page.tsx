    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left animate-in fade-in duration-500">
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-white rounded-xl border border-slate-200 p-8 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-900">Account Settings</h2>
            <button onClick={() => alert('Saved!')} className="bg-[#3C50E0] text-white px-6 py-2 rounded-md font-bold text-sm hover:opacity-90 transition">
              Save Changes
            </button>
          </div>
            ...
            {[
              ...
              <div key={input.name} className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full p-4 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-700 text-sm focus:outline-none focus:border-[#3C50E0] focus:bg-white transition-all shadow-inner"
                />
              </div>
            ))}
          </div>
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 className="font-extrabold mb-6 border-b pb-4 text-slate-900">Quick Actions</h3>
          <div className="space-y-3">
            {["My Saved Outfits", "Try-On Room", "Order History", "Change Password"].map(item => (
              <div key={item} className="flex justify-between items-center p-4 bg-slate-50 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-100 cursor-pointer transition group">
                {item} <RightOutlined className="text-slate-300 group-hover:text-[#3C50E0]" style={{ fontSize: '12px' }} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 className="font-extrabold mb-4 border-b pb-4 text-slate-900">Account Info</h3>
          <div className="space-y-4 text-sm font-bold">
            <div className="flex justify-between">
              <span className="text-slate-400">Status</span>
              <span className="text-emerald-500 flex items-center gap-1"><CheckCircleFilled className="text-sm"/> Verified</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Total Orders</span>
              <span className="text-slate-900">45 Items</span>
            </div>
  ...
  return (
    <div className="text-left animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl font-extrabold text-slate-900">Saved Outfits ( 3 )</h2>
        <button className="text-[#3C50E0] font-bold text-sm flex items-center gap-1 hover:underline">
        ...
        {outfits.map((o, i) => (
          <div key={i} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden group">
            ...
            <div className="p-8 space-y-4">
              <h3 className="text-2xl font-extrabold group-hover:text-[#3C50E0] transition-colors text-slate-900">{o.title}</h3>
              ...
              <button className="w-full bg-[#3C50E0] text-white py-3.5 rounded-lg font-bold text-sm mt-4 hover:opacity-90 active:scale-[0.98] transition-all shadow-md">
                Try On Room
              </button>
            </div>
          </div>
...
function ShopsSection() {
  ...
  return (
    <div className="space-y-6 text-left animate-in fade-in duration-500">
      <h2 className="text-2xl font-extrabold mb-8 text-slate-900">Liked Shops ( {shops.length} )</h2>
      {shops.map((shop, i) => (
        <div key={i} className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8 relative group hover:border-[#3C50E0]/30 transition-all">
          ...
          <div className="flex-1 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="bg-emerald-50 text-emerald-600 text-[10px] px-2 py-0.5 rounded font-bold border border-emerald-100 italic">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <button className="flex-1 bg-[#3C50E0] text-white px-8 py-3 rounded-lg font-bold text-xs flex items-center justify-center gap-2 shadow-sm hover:opacity-90">Visit Shop <ExportOutlined /></button>
            <button className="flex-1 border border-slate-200 text-slate-600 px-8 py-3 rounded-lg font-bold text-xs flex items-center justify-center gap-2 hover:bg-slate-50 shadow-sm transition-all"><MessageOutlined /> Contact</button>
          </div>
        </div>
      ))}
    </div>
...
// --- COMPONENT CHÍNH (PHẢI LÀ EXPORT DEFAULT) ---
export default function ProfilePage() {
  const [activeTab, setActiveTab] = useState('about');

  return (
    <div className="min-h-screen bg-[#F1F5F9] pb-20 font-sans text-left text-slate-900">
      {/* --- HEADER PROFILE --- */}
            ...
            <div className="flex-1 w-full">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                  <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">Linh Trang</h1>
                ...
                <button className="bg-[#3C50E0] text-white px-5 py-2.5 rounded-md flex items-center gap-2 font-bold text-sm hover:opacity-90 transition shadow-sm self-start md:self-center">
              ...
              <div className="flex flex-wrap gap-10 mt-8 pt-6 border-t border-slate-100">
                ...
                <div className="text-left"><p className="text-2xl font-extrabold text-[#3C50E0]">3</p><p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Outfits</p></div>
                ...
                <div className="text-left"><p className="text-2xl font-extrabold text-[#3C50E0]">2024</p><p className="text-[10px] text-slate-400 uppercase font-black tracking-widest">Member Since</p></div>
              </div>
            </div>
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              className={`pb-4 text-sm font-bold transition-all border-b-4 ${
                activeTab === tab.id ? "border-[#3C50E0] text-[#3C50E0]" : "border-transparent text-slate-400 hover:text-slate-500"
              }`}
            >
              {tab.name}
            </button>
